...
# To install Fmt.jl, please do pkg> add https://github.com/bicycle1885/Fmt.jl
@time using Fmt
...
# %%
function anim_sol2D(sol; L = 101, clim=(-0.03, 0.03), color=:bwr, bg=:lightgray)
    @unpack prob, u = sol
    @unpack tspan, p = prob
    @unpack x, y = p
    ...
    @gif for t in ts
        ...
        plot!(size=(520, 540), xlim=extrema(x), ylim=extrema(y), aspectratio=1)
        next!(prog)
    end
end
...
function anim_sol(sol; L=151, fps=10, fn="tmp.gif",
        color=:bwr, bg=:lightgray, camera=(30, 85), size=(720, 540), kwargs...)
    @unpack prob, u = sol
    @unpack tspan, p = prob
    @unpack x, y = p
...
sol = nothing; GC.gc()
@time sol = solve(prob; saveas=0.01)
...
# %%
plot_sol(sol, 0.3; zlim=(-0.03, 0.03))

# %%
anim_sol(sol; fn="wave eq on pentagon neumann.gif", zlim=(-0.03, 0.03))
...
# %%
anim_sol(sol; fn="wave eq on pentagon dirichlet.gif", zlim=(-0.03, 0.03))
