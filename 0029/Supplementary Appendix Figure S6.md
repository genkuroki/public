...
パラメータ $\theta$ に関する事後分布が得られているとき, 仮説 $\theta = \theta_0$ の(両側検定の)P値の類似物を, 事後分布において $\theta\le\theta_0$ が成立する確率と $\theta\ge\theta_0$ が成立する確率の小さい方の2倍で定義することができる. (これはClopper-Pearsonの信頼区間に対応するP値の類似であり, 別のスタイルによる定義もある.)
