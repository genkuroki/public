...
#
# __IRRの点推定値:__ $\IRR$ (incidence rate ratio, 単位時間あたりの発症数の期待値の比)の点推定値:
#
# $$
...
# $$
#
# __統計モデル:__ ワクチン群とプラセボ群における発症が, 単位時間あたりの発生数の期待値がそれぞれ $\lambda_1, \lambda_0$ のPoisson過程に従ってランダムにかつ独立に起こっているというモデルを考える. 
#
# すなわち, このモデル内におけるワクチン群とプラセボ群での発症人数 $C_1, C_0$ (これらは確率変数)はそれぞれ期待値 $T_1\lambda_1$, $T_0\lambda_0$ のPoisson分布に独立に従うと考えることになる:
#
# $$
...
# $$
#
# このモデルにおける $\IRR$ (incidence rate ratio, 単位時間あたりの発症数の期待値の比)と $\VE$ (vaccine efficacy)が次のように定義される:
#
# $$
...
#
# __有意水準と信頼度:__ 以下において, 有意水準 $0\le\alpha\le 1$ を固定する. このとき $1-\alpha$ は信頼度(信頼水準)と呼ばれる.
#
...
#
# __Clopper-Pearsonの信頼区間のベータ分布表示:__ 二項分布の累積分布函数がベータ分布の累積分布函数で表されるという数学的結果
#
# $$
# \begin{aligned}
# &
...
#
# __IRRとVEに関するClopper-Pearsonの信頼区間:__ $\IRR$ と $\VE$ を $\pi$ で表す公式
#
# $$
# \IRR = \frac{\pi}{r(1-\pi)}, \quad
# \VE = 1 - \frac{\pi}{r(1-\pi)}.
