...
# ゆえに, $\bar X-\bar Y$は期待値$\mu_X-\mu_Y$と分散$\dfrac{\sigma^2_X}{m}+\dfrac{\sigma^2_Y}{n}$を持つ正規分布に従う.
#
# (3) $(m-1)S^2_X/\sigma^2_X$は自由度$m-1$の$\chi^2$分布に従い, $(n-1)S^2_Y/\sigma^2_Y$は自由度$n-1$の$\chi^2$分布に従う.
#
# 自由度$\nu$の$\chi^2$分布の期待値と分散はそれぞれ$\nu$と$2\nu$である
#
# 上の(2)より,
# \begin{align*}
# \Delta\mu = \mu_X - \mu_Y, \quad
...
# が従う分布は標準正規分布で近似される.
#
# 標本サイズ$m,n$が十分大きいならば, 不偏分散$S^2_X, S^2_Y$で母分散$\sigma^2_X, \sigma^2_Y$を近似するので, $Z$は
# \begin{align*}
# T = \frac{\bar X-\bar Y-\Delta\mu}{\sqrt{\dfrac{S^2_X}{m}+\dfrac{S^2_Y}{n}}}
# \end{align*}
# で近似される.
#
# このことから, 標本サイズ$m,n$を十分に大きい場合には, Welchのt検定におけるt分布を標準正規分布で置き換えた検定法を使用できることが分かる.

# %% [markdown]
# さらに, Welchのt検定で使う自由度は$\min(m-1, n-1)$以上になるので, Welchのt検定で使う自由度は標本サイズ$m,n$を大きくすると大きくなり, 自由度が大きなt分布は標準正規分布で近似されることから, 標本サイズ$m,n$が十分大きいならば, t分布を使った通常のWelchのt検定の結果と, Welchのt検定で使うt分布を標準正規分布で置き換えることによって得られた検定の結果は近似的に等しくなる.
#
# これで, 母集団分布に応じて標本サイズ$m,n$が十分大きくすれば, 非正規母集団の場合であってもWelchのt検定を十分に使用できることが分かった.
#
...
#
# ゆえに, $X_i$が従う分布が非正規分布で尖度がゼロでないときの不偏分散$S^2_X$が従う分布の分散は, $X_i$が従う分布が正規分布のときの$S^2_X$が従う分布の分散と異なる値になり, 特に$X_i$が従う分布が非正規分布で尖度がゼロでないときの$\dfrac{(m-1)S^2_X}{\sigma^2_X}$が従う分布は正規分布の場合の自由度$m-1$の$\chi^2$分布で近似されなくなる.
#
# このことから, 母集団分布が正規分布で近似されている保証がない場合には, $\dfrac{(m-1)S^2_X}{\sigma^2_X}$が従う分布が自由度$m-1$の$\chi^2$分布になることを使う検定法は使用しない方がよい. 例えば, 1標本での母分散に関する$\chi^2$検定や2標本での母分散の比に関するF検定は使わない方がよい.
