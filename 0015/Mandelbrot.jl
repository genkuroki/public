...
# ╔═╡ b8c3fa60-f739-11eb-3acf-a7d6ce8e5a11
begin
	using Plots
	gr(fmt=:png)
end
...
begin
	n = 2^8
	x0, x1 = -0.714689, -0.714679
	y0, y1 =  0.299872,  0.299882
	x = range(x0, x1; length=n)
	y = range(y0, y1 ; length=n)
	c = complex.(x', y)
end;
...
begin
	x32 = range(Float32(x0), Float32(x1); length=n)
	y32 = range(Float32(y0), Float32(y1); length=n)
	c32 = complex.(x32', y32)
end;
...
plotmandelbrot(collect(mandelbrot.(cu(c32))))
...
plotmandelbrot(mandelbrot_threads(c))
