...
function plot_t_tests(;
    ...
    αs = 10.0 .^ range(log10(0.002), log10(1), 1000)
    Q = plot()
    plot!(αs, α -> ECDF(pval_Welch, α); label="Welch t-test")
    plot!(αs, α -> ECDF(pval_Student, α); label="Student t-test", ls=:dot)
    plot!(αs, identity; label="", ls=:dot, c=:black, alpha=0.3)
    _ticklog10 = Any[0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1]
    ticklog10 = (_ticklog10, string.(_ticklog10))
    plot!(; xtick=ticklog10, ytick=ticklog10, xrotation=90)
    plot!(; xguide="nominal significance level α",
        yguide="probability of p-value ≤ α")
    plot!(; legend=:outertop, legendfontsize=10)
    ...
    plot!(; xscale=:log10, yscale=:log10, size=(350, 400))
end
...
P = bar(distx; alpha=0.3, label="distx", ylim=(-0.05, 0.5), xlim=(-4, 10))
Q = bar(disty; alpha=0.3, label="disty", ylim=(-0.05, 0.5), xlim=(-4, 10))
plot(P, Q; size=(600, 200)) |> display
