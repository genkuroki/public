...
Y = [12, 12, 14, 15, 18, 18, 19, 22, 28, 29, 30, 32, 37, 40, 43, 45, 45, 46, 46, 47, 49, 50, 55, 58, 68, 79, 81, 89, 161, 732]

P = dotplot([X, Y]; label="", xtick=(1:2, ["X", "Y"]), msc=:auto, title="data")
Q = dotplot(vec(X .- Y'); xtick=(1:1, ["X − Y"]), label="", c=3, msc=:auto, ms=2, ma=0.7)
...
println("Brunner-Munzel: ", "point estimate = ", r(HL), ",  95% confidence interval = ", r.(CI_BM))
println("Welch t-test: ", "point estimate = ", r(DM), ",  95% confidence interval = ", r.(CI_W))

plot(a -> pvalue_brunner_munzel(X, Y .+ a), -100, 80; label="Brunner-Munzel", c=1)
...
plot!(xtick=-20:5:30, ytick=0:0.05:1)
