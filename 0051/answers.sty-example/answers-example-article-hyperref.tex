...
\newtheoremstyle{problemwithanswer}
  {10pt}{10pt}  % 上下の空白
  {\normalfont} % 本文のフォント
  {0pt}         % インデント
  {\bfseries}   % タイトルのフォント
  {}           % タイトルの後の記号
  {5pt plus 1pt minus 1pt} % タイトルと本文の間のスペース
  {\thmname{#1}\thmnumber{ #2}\thmnote{ (#3)} \hyperref[answer:\theproblem]{[解答例p.\pageref{answer:#2}]}\label{problem:#2}} % タイトルのフォーマット
\theoremstyle{problemwithanswer}
...
\renewcommand{\showanswerlabel}[1]{\hspace{5.4pt}%
  \textbf{\hyperref[problem:#1]{[p.\pageref{problem:#1}] 問題#1}\label{answer:#1}解答例.}}
